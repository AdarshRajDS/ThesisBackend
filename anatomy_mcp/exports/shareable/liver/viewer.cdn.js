...
const modelUrl          = params.get("model")
  || window.ANATOMY_MODEL
  || fallbackModel;
const annotationsUrl    = params.get("annotations")
  || window.ANATOMY_ANNOTATIONS
  || fallbackAnno;
...
const pageTitleParam = params.get("title") || window.ANATOMY_TITLE;
 ...
 *  Returns the world-space offset that was subtracted (Three.js coords). */
function centerModelAtOrigin(root, box) {
  const c = box.getCenter(new THREE.Vector3());
  if (c.lengthSq() < 1e-12) return new THREE.Vector3();
  root.position.sub(c);
...
function fitCameraToBounds(camera, controls, box, frame, aspect) {
  ...
  // Fit the full bounding sphere with margin so the organ sits in-frame.
  const dist = Math.max(
    radius / Math.max(Math.sin(vFov / 2), 1e-4),
    radius / Math.max(Math.sin(hFov / 2), 1e-4),
  ) * 1.45;

  camera.aspect = safeAspect;
  camera.up.copy(up);
  // Mild superior offset keeps anterior anatomy readable without pushing
  // the model out of the top of the viewport.
  ...
  camera.far = Math.max(radius * 200, dist * 20);
...
async function init() {
  ...
  // Z-Anatomy parts often sit far from the world origin — recenter so camera,
  // lights, and labels all share the same origin and stay mid-viewport.
  let box = computeModelBounds(gltf.scene);
  const modelOffset = centerModelAtOrigin(gltf.scene, box);
  box = computeModelBounds(gltf.scene);
  ...
  function viewportAspect() {
    const w = Math.max(container.clientWidth || 0, 1);
    const h = Math.max(container.clientHeight || 0, 1);
    return w / h;
  }
  // Mutable home pose — refreshed after layout settles and on Reset view.
  const viewHome = {
    center: center.clone(),
    radius: 0.05,
    homePos: new THREE.Vector3(0, 0, 1),
    homeUp: frame.up.clone(),
  };
  function frameCameraToModel() {
    const pose = fitCameraToBounds(camera, controls, box, frame, viewportAspect());
    viewHome.center.copy(pose.center);
    viewHome.radius = pose.radius;
    viewHome.homePos.copy(pose.homePos);
    viewHome.homeUp.copy(pose.homeUp);
    return pose;
  }
  frameCameraToModel();
  ...
  // Blender (x, y, z) → Three (x, z, -y). Absolute world anchors are shifted by
  // the same modelOffset applied when recentering the GLB at the origin.
  function anchorFromJson(item) {
    ...
    if (Array.isArray(item.anchor_world)) {
      ...
      return new THREE.Vector3(x, z, -y).sub(modelOffset);
    }
    return null;
  }
  ...
  if (controlsHost) {
    ...
    function setView(dir, upv) {
      const aspectNow = viewportAspect();
      const vFov = THREE.MathUtils.degToRad(camera.fov);
      const hFov = 2 * Math.atan(Math.tan(vFov / 2) * aspectNow);
      const dist = Math.max(
        viewHome.radius / Math.max(Math.sin(vFov / 2), 1e-4),
        viewHome.radius / Math.max(Math.sin(hFov / 2), 1e-4),
      ) * 1.45;
      controls.target.copy(viewHome.center);
      camera.up.copy(upv);
      camera.position.copy(viewHome.center).addScaledVector(dir.clone().normalize(), dist);
      camera.lookAt(viewHome.center);
      controls.update();
    }
    const views = [
  ...
  // ── Label layout ─────────────────────────────────────────────────────────────
  // Project each structure anchor onto the screen and place the chip just above
  ...
  function labelAnchorWorld(entry) {
    if (entry.meshObj) {
      ...
      const meshCenter = entry.centerLocal
        ? entry.meshObj.localToWorld(entry.centerLocal.clone())
        : new THREE.Box3().setFromObject(entry.meshObj).getCenter(new THREE.Vector3());
  ...
  function layoutLabels() {
    ...
    for (const entry of labelEntries) {
      ...
      // Discard only points clearly behind the near plane / camera.
      if (v.z > 1) continue;

      const x = (v.x * 0.5 + 0.5) * W;
      const y = (-v.y * 0.5 + 0.5) * H;
      // Skip anchors far outside the frustum — they belong to hidden faces.
      if (x < -40 || x > W + 40 || y < -40 || y > H + 40) continue;

      const ch = entry.element.offsetHeight || 28;
      const cw = Math.min(entry.element.offsetWidth || 120, 180);
      // Keep the chip just above the surface dot, clamped inside the viewport
      // so leader lines stay short and labels stay attached to the organ.
      let lx = x;
      let ly = y - ch - 8;
      lx = Math.min(Math.max(lx, pad + cw * 0.5), W - pad - cw * 0.5);
      ly = Math.min(Math.max(ly, pad), H - pad - ch);
      ...
      entry.line.setAttribute("y2", ly + ch);
  ...
  let didInitialReframe = lastW > 10 && lastH > 10;
  function onViewportResize() {
    ...
    // First time the canvas gets a real size (iframe / delayed layout), reframe.
    if (!didInitialReframe || (Math.abs(w - lastW) > 40 || Math.abs(h - lastH) > 40)) {
      if (!didInitialReframe) {
        frameCameraToModel();
        didInitialReframe = true;
      }
    }
    lastW = w;
    lastH = h;
  ...
  // Catch late layout after fonts/sidebar paint.
