...
def _metrics_row(qid: str, question: str, gold: str, item: dict) -> str:
    q = _cell(question, 90)
    g = _cell(gold, 80)
    ans = _system_answer(item)
    return f"| {qid} | {q} | {g} | {ans} | | | | | | | | | |"


def build_metrics_table(rows: list[dict], id_prefix: str) -> str:
    if len(rows) != len(PROFESSOR_META):
        raise ValueError(f"Expected {len(PROFESSOR_META)} rows, got {len(rows)}")
    header = (
        "| ID | Question | Expected key points | System answer | "
    ...
    for i, (item, (_, gold)) in enumerate(zip(rows, PROFESSOR_META), 1):
        ...
        body.append(_metrics_row(qid, item["question"], gold, item))
    return header + "\n" + "\n".join(body)
...
{build_metrics_table(local, "LM-R")}

### 9.1.2 Cloud exploratory run (historical comparison)

{build_metrics_table(cloud, "EXP-R")}
...
def main() -> None:
    ...
    section = build_section(cloud, local)
