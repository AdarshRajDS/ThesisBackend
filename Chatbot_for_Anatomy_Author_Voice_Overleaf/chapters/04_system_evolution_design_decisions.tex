...
Four evidence levels are used. Project notes and formative evaluations support the historical account. Repository files show implementation. Call-graph inspection is required before a mechanism can be described as part of an endpoint, and a controlled run is required before any performance effect is claimed. The hybrid retriever is a useful example: \codepath{HybridRetriever} was connected in commit \texttt{5089fcc} and was the preferred P1 route, while its improvement over dense-only retrieval remains unevaluated. Historical material comes mainly from \codepath{project_resources/THESIS_SYSTEM_DESIGN_AND_MCP.md}, \codepath{project_resources/EVALUATION_DATA_SUMMARY.md}, and \codepath{project_resources/PROFESSOR_REQUIREMENTS_TRACEABILITY.md}; implementation statements are checked against the repository.
...
The implementation account in \cref{chap:implementation} must stay aligned with the evaluated runtime. Hybrid retrieval wiring on \codepath{/rag/ask} after commit \texttt{5089fcc} is verified as the preferred production path. World-knowledge consent, strict evidence gating, post-generation citation verification, and complete route-separation controls remain incomplete or unverified and must not be described as active production behaviour until their call paths have been confirmed.
...
Several corrections remain incomplete at runtime or in evaluation. Hybrid retrieval was connected on the production endpoint after \texttt{5089fcc}, but world-knowledge consent, strict evidence gating, and post-generation citation verification remained incomplete, and frontend compatibility behaviour weakens the strict export contract. These gaps are part of the result because they identify what must change before the prototype can be evaluated as the system shown in the architecture.
